...
pair<pair<index_elt*,size_t>, char*> parse(string filename, size_t max_size) {
  startTime();
  array_imap<char> Str = read_string_from_file(filename, 0, max_size);
  size_t n = Str.size();
  nextTime("read file");
  ...
  array_imap<char*> W = string_to_words(Str);
  size_t num_words = W.size();
  nextTime("find words");

  array_imap<bool> start_flag(num_words);
  ...
  array_imap<size_t> I = pack_index<size_t>(start_flag);
  ...
  return make_pair(make_pair(KV,total_pairs),Str.get_array());
}
